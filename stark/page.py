...
class MyPage(object):
    def __init__(self, page_num, all_data_amount, request, per_page_data=10, page_show_tags=9):
        ...
        # 将QueryDict类型的字典拷贝一份，会执行__deepcopy__将mutable置为True，
        # 就可以对拷贝的那一份进行修改了
        import copy
    ...
    def ret_html(self):
        ...
        front_page_tag = '<li><a href="?page={}">&laquo;</a></li>'.format(self.params.urlencode)
        self.params['page'] = self.page_num + 1
        next_page_tag = '<li><a href="?page={}">&raquo;</a></li>'.format(self.params.urlencode)

        page_tag_html = ''
        for i in range(show_tags_left, show_tags_right + 1):
            self.params['page'] = i
            # params的urlencode方法会返回一个page=1&a=2&b=3这样的字符串，
            # 其实django=WSGI在收到的url上就是这样的字符串，变成字典后存到request.GET中做为用户访问信息的一部分
            if i == self.page_num:
